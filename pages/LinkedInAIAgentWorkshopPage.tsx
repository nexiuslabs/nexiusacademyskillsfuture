    ...
    <>
      ...
      <main className="min-h-screen overflow-hidden bg-[#031716] text-white">
        <section className="relative border-b border-accent/15">
          ...
          <div className="relative mx-auto max-w-7xl px-5 pb-20 pt-7 sm:px-8 lg:px-10 lg:pb-28">
            ...
            <div className="grid items-center gap-14 lg:grid-cols-[1.12fr_.88fr]">
              <div>
                <div className="mb-7 inline-flex items-center gap-2 rounded-full border border-accent/50 bg-accent/10 px-4 py-2 text-xs font-bold uppercase tracking-[.18em] text-accent">
                  <span className="h-2 w-2 rounded-full bg-accent" /> 2-hour hands-on workshop
                </div>
                <h1 className="max-w-4xl text-balance text-5xl font-black leading-[.98] tracking-[-.045em] sm:text-6xl lg:text-7xl">
                  Build Your Own <span className="text-accent">LinkedIn AI Agent</span>
                </h1>
                ...
                <div className="mt-10 flex flex-wrap gap-3 text-sm text-slate-300">
                  <span className="flex items-center gap-2 rounded-full bg-white/5 px-4 py-2"><CalendarDays className="h-4 w-4 text-accent" /> 16 September</span>
                  <span className="flex items-center gap-2 rounded-full bg-white/5 px-4 py-2"><Clock3 className="h-4 w-4 text-accent" /> 12:30pm–2:30pm</span>
                  <span className="flex items-center gap-2 rounded-full bg-white/5 px-4 py-2"><MapPin className="h-4 w-4 text-accent" /> Hotel Boss, Level 4</span>
              ...
              <div className="relative mx-auto w-full max-w-lg">
                ...
                <div className="relative rounded-3xl border border-accent/25 bg-[#062825]/90 p-4 shadow-2xl shadow-black/50">
                  ...
                  <div className="space-y-4 p-3 sm:p-6">
                    <div className="rounded-2xl border border-accent/20 bg-accent/5 p-4"><span className="text-xs font-bold uppercase tracking-widest text-accent">Your expertise</span><p className="mt-2 font-semibold">Voice · Audience · Content pillars</p></div>
                    ...
                    <div className="flex justify-center text-accent"><ArrowRight className="h-6 w-6 rotate-90" /></div>
                    <div className="rounded-2xl bg-gradient-to-r from-accent to-[#008f83] p-5 text-center font-black text-primary shadow-lg">Publish-ready LinkedIn draft</div>
                  </div>
                </div>
              </div>
            </div>
        ...
        <section className="mx-auto grid max-w-7xl gap-12 px-5 py-20 sm:px-8 lg:grid-cols-[1fr_.82fr] lg:px-10 lg:py-28">
          <div>
            <p className="font-bold uppercase tracking-[.2em] text-accent">What you will build</p>
            ...
            <div className="mt-10 grid gap-4 sm:grid-cols-2">
              {outcomes.map((outcome) => <div key={outcome} className="flex gap-3 rounded-2xl border border-white/10 bg-white/[.035] p-4"><Check className="mt-0.5 h-5 w-5 shrink-0 rounded-full bg-accent p-1 text-primary" /><span className="font-semibold leading-6">{outcome}</span></div>)}
            </div>
          ...
          <aside id="register" className="scroll-mt-8">
            <div className="sticky top-8 rounded-3xl bg-white p-6 text-slate-900 shadow-2xl sm:p-8">
              {status === 'success' ? (
                ...
                <>
                  <div className="flex items-center gap-2 font-bold text-[#007f75]"><Sparkles className="h-5 w-5" /> Workshop registration</div>
                  ...
                  <div className="mt-4 space-y-2 rounded-2xl bg-[#e8fffc] p-4 text-sm text-slate-700">
                    <p className="flex gap-2"><CalendarDays className="h-5 w-5 shrink-0 text-[#007f75]" /><strong>16 September, 12:30pm–2:30pm</strong></p>
                    <p className="flex gap-2"><MapPin className="h-5 w-5 shrink-0 text-[#007f75]" /><span><strong>Hotel Boss, Level 4</strong><br />500 Jalan Sultan Road, Singapore</span></p>
                    <p className="border-t border-accent/25 pt-3 leading-6"><strong>Complimentary admission.</strong> Lunch is not included and will be at each participant’s own expense.</p>
                  </div>
                  ...
                  <form onSubmit={handleSubmit} className="mt-7 space-y-4">
                    ...
                    <button disabled={status === 'submitting'} className="flex w-full items-center justify-center gap-2 rounded-xl bg-accent px-5 py-4 font-black text-primary transition hover:bg-[#00b4a5] disabled:cursor-wait disabled:opacity-60">{status === 'submitting' ? 'Submitting…' : 'Register for workshop'}<ArrowRight className="h-5 w-5" /></button>
