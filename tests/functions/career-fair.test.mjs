...
test('publishes the three-minute scenario diagnostic and course facts without the removed pathway section', () => {
  for (const phrase of ['3-minute gap test','9 & 16 October 2026']) assert.match(page, new RegExp(phrase.replace(/[&/]/g, '\\$&')));
  assert.doesNotMatch(page, /September cohort|18 & 25 September 2026/);
