...
const buildCertificatePdf = (details: CertificateDetails, score: number, total: number) => {
  const issuedOn = new Intl.DateTimeFormat('en-SG', {
    day: '2-digit',
    month: 'short',
    year: 'numeric',
  }).format(new Date());
  const courseLines = wrapText(details.courseName.toUpperCase(), 33).slice(0, 4);
  const safeName = details.recipientName.trim() || 'Participant';
  const dateLines = details.courseDates.length ? details.courseDates : ['Date to be confirmed'];
  const safeTrainerName = details.trainerName.trim() || 'Trainer to be confirmed';
  const content = [
...
const downloadCertificate = (details: CertificateDetails, score: number, total: number) => {
  const blob = buildCertificatePdf(details, score, total);
  const url = URL.createObjectURL(blob);
  const filenameName = details.recipientName.trim().toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '') || 'participant';
          ...
          <div>
            ...
            <div className="mt-6 rounded-2xl border border-primary/10 bg-neutral p-5">
              ...
              <p className="mt-2 text-sm leading-relaxed text-gray-600">
                Enter the learner name, course name, course dates, and trainer name exactly as they should appear on the PDF certificate.
              </p>
